...
import day18.Duet.{firstRcv, secondSent}
...
class DuetTest extends FlatSpec {
  ...
  it should "solve my input" in {
    assert(firstRcv(myInput) == 3188)
  }

  "Second sent" should "be number of messages sent by program 1" in {
    assert(secondSent(
      """snd 1
        |snd 2
        |snd p
  ...
  it should "solve my input" in {
    assert(secondSent(myInput) == 7112)
  }

  private val myInput: String =
    """set i 31
      |set a 1
      |mul p 17
      |jgz p p
