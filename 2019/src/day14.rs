...
pub fn part1(input: &str) -> usize {
    ...
    temp((String::from("FUEL"), 1), &data).get("ORE").cloned().unwrap_or(0)
}

type Res = (String, usize);
type Ing = Vec<Res>;
fn temp(resource: Res, data: &[(Res, Ing)]) -> HashMap<String,usize> {
    // find ingredients for the resource
    //   while ingredients expandable
    //      expand the largest resource
    //      try to use leftovers with rest of ingredients
    let mut ingredients: HashMap<String, usize> = data
        .iter()
        .find(|(r, _)| r.0 == resource.0)
        .map(|(_, i)| i.iter().cloned().collect::<HashMap<_, _>>())
