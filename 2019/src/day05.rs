...
pub fn part1(input: &str) -> i32 {
    execute(&mut numbers(input), 1).last().cloned().unwrap()
}

pub fn part2(input: &str) -> i32 {
    execute(&mut numbers(input), 5).last().cloned().unwrap()
}
...
fn execute(program: &mut [i32], input: i32) -> Vec<i32> {
    let mut output = vec![];
    let mut pointer = 0;
    loop {
...
mod spec {
    ...
    fn passes_diagnostic_tests() {
        let output = execute(&mut numbers(INPUT), 1);
